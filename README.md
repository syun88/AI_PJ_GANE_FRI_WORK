...
```mermaid
flowchart TD
    A[ゲーム開始] --> B[配置カードの決定・配布]
    B --> C[ダンジョン生成（9部屋レイアウト）]
    C --> D[プレイヤー初期位置を設定]
    D --> E[初期状態を整備（速度1・幽霊未出現など）]
    E --> F[鍵/ダミー鍵/効果アイテムをランダム配置]
    F --> G[安全エリア・凍結ターン等のステート初期化]
    G --> H[ルールとコマンド一覧を提示]
    H --> I[現在の状態を表示]
    I --> J[ターン開始]
    J --> K{ゲーム継続条件を満たす？}
    ...
    L --> M2[通常移動]
    M2 --> N1{移動可能？}
    N1 -- いいえ --> O1[失敗としてログ→ターン継続]
    O1 --> L
    N1 -- はい --> P1[位置更新・total_steps加算]
    P1 --> Q1{探索マスに到達？}
    Q1 -- はい --> R1[探索イベントを解決→アイテム抽選]
    R1 --> S1{入手する？}
    S1 -- はい --> T1[インベントリに追加]
    S1 -- いいえ --> U1[床に残す]
    T1 --> V1
    ...
    AC --> AD{接触した？}
    AD -- はい --> Z1
    AD -- いいえ --> AE[ターン終了処理（ログ整理・効果ターン減算）]
    AE --> J
```
